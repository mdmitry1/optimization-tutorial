...
def run_optimization(rootpath, max_depth, n_estimators, min_samples_split) -> int:
    ...
    df = pd.read_csv(rootpath + '/s2_tx_anonym.csv.gz', sep=',')
    ...
    class MaxOverFixedParamsProb(Problem):
        ...
        def _evaluate(self, X, out, *args, **kwargs):
            # X is a 2D array where each row is a solution [p0, p1, p2, p3]
            ...
            for i in range(n_solutions):
                ...
                X_full = np.zeros((n_fixed_combos, 7))
                X_full[:, 0:3] = unique_fixed_combinations  # CH, RANK, Byte
                X_full[:, 3:7] = np.tile(X[i, :], (n_fixed_combos, 1))  # p0, p1, p2, p3
    ...
    print("="*100)
    print(f"{'#':<4} {'p0':<10} {'p1':<10} {'p2':<10} {'p3':<10} {'max(o0)':<12} {'max(o1)':<12}")
    print("-"*100)
